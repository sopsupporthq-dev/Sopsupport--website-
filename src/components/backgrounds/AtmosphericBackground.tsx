...
export function AtmosphericBackground({ variant = "default" }: { variant?: BgVariant }) {
  ...
  // Phones and other low-power/coarse-pointer devices get a lighter version
  // of the same background: the parallax drift is skipped and the blur
  // radii are reduced ~30%. Colors, positions, sizes, and opacity are all
  // identical to the desktop version, so the look stays the same — it's just
  // cheaper for a weaker GPU to render.
  const isLowPower =
    typeof window !== "undefined" &&
    window.matchMedia("(pointer: coarse), (max-width: 640px)").matches;
  const blurScale = isLowPower ? 0.7 : 1;

  useEffect(() => {
    if (isLowPower) return; // no parallax on low-power devices
    ...
    const tick = () => {
      ...
      const delta = targetYRef.current - scrollYRef.current;
      els.forEach(el => {
        const offset = -(scrollYRef.current * parseFloat(el.dataset.depth ?? "0"));
        el.style.setProperty("--px", `${offset}px`);
      });
      // Once the eased value has effectively caught up to the scroll
      // position, stop the loop instead of running requestAnimationFrame
      // forever — this was previously ticking at 60fps indefinitely, even
      // while the page sat idle, burning CPU/battery for no visible effect.
    ...
    >
      ...
      {cfg.glows.map((g, i) => (
        <div key={i} data-depth={g.depth} className="absolute rounded-full" style={{
          ...
          filter: `blur(${g.blur * blurScale}px)`,
          animation: `bgGlow${(i % 4) + 1} ${g.duration}s ease-in-out infinite`,
          transform: isLowPower ? undefined : "translateY(var(--px, 0px))",
        }} />
      ))}
      ...
      <div
        ...
        style={{ height: "520px", background: "linear-gradient(to bottom, transparent 0%, rgba(0,0,0,0.4) 35%, rgba(0,0,0,0.75) 65%, rgba(0,0,0,0.96) 100%)", zIndex: 1 }}
      />
      <style>{`
